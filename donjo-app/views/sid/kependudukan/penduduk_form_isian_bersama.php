...
<div class="row">
  <div class="col-sm-12">
    <div class='form-group'>
      <label for="validate">Validasi Dengan Data DISDUKCAPIL </label>
      <span id="validate" class="input-group-btn">
        <button type='button' class='btn btn-social btn-flat btn-info btn-sm' onclick="get_penduduk_detail()"><i class='fa fa-database'></i>Proses Pengecekkan</button>
        <div id="loading" style="position:absolute;bottom:0px;right:-20px;z-index:999;display:none">
      ...
      <script>
        function get_penduduk_detail() {
          var nik = document.getElementById("nik").value;
          if (nik == "") {
            alert('Maaf silahkan isi Nomor NIK terlebih dahulu');
          } else {
            $('#loading').show();
            $.ajax({
              type: "GET",
              url: "<?php echo site_url('api/get_penduduk/'); ?>" + nik,
              data: "",
              dataType: 'json',
              success: function(data) {
                $.each(data.content, function(i, penduduk) {
                  if (penduduk.RESPON == "Data Tidak Ditemukan") {
                    $('#loading').hide();
                    alert('Maaf data penduduk tidak diketemukan,periksa kembali nomor NIK');
                  } else {
                    $("#nama").val(penduduk.NAMA_LGKP);
                    $("#tempatlahir").val(penduduk.TMPT_LHR);
                    $("#tgl_1").val(penduduk.TGL_LHR);
                    $("#alamat_sebelumnya").val(penduduk.ALAMAT + " " + penduduk.KEL_NAME + " " + penduduk.KEC_NAME + " " + penduduk.KAB_NAME);
                    $("#alamat_sekarang").val(penduduk.ALAMAT + " " + penduduk.KEL_NAME + " " + penduduk.KEC_NAME + " " + penduduk.KAB_NAME);
                    $("#warganegara_id option[value=1]").attr('selected', 'selected');
                    set_golongan_darah(penduduk.GOL_DARAH);
                  }
                });
                $('#loading').hide();
              },
              error: function(data, textStatus, jqXHR) {
                alert('Maaf Koneksi Error' + " " + textStatus);
              }
            });
          }
        }

        function set_golongan_darah(input) {
          var input = input;

          if (input == "A") {
            $("#golongan_darah_id option[value=1]").attr('selected', 'selected');
          } else if (input == "B") {
            $("#golongan_darah_id option[value=2]").attr('selected', 'selected');
          } else if (input == "AB") {
            $("#golongan_darah_id option[value=3]").attr('selected', 'selected');
          } else if (input == "O") {
            $("#golongan_darah_id option[value=4]").attr('selected', 'selected');
          } else if (input == "A+") {
            $("#golongan_darah_id option[value=5]").attr('selected', 'selected');
          } else if (input == "A-") {
            $("#golongan_darah_id option[value=6]").attr('selected', 'selected');
          } else if (input == "B+") {
            $("#golongan_darah_id option[value=7]").attr('selected', 'selected');
          } else if (input == "B-") {
            $("#golongan_darah_id option[value=8]").attr('selected', 'selected');
          } else if (input == "AB+") {
            $("#golongan_darah_id option[value=9]").attr('selected', 'selected');
          } else if (input == "AB-") {
            $("#golongan_darah_id option[value=10]").attr('selected', 'selected');
          } else if (input == "O+") {
            $("#golongan_darah_id option[value=11]").attr('selected', 'selected');
          } else if (input == "O-") {
            $("#golongan_darah_id option[value=12]").attr('selected', 'selected');
          } else {
            $("#golongan_darah_id option[value=13]").attr('selected', 'selected');
          }
        }
      </script>
    </div>
